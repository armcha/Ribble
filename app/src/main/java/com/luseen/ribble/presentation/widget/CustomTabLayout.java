...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!//
///THIS CLASS IS COPY OF ORIGINAL TABLAYOUT, JUST CHANGED INDICATOR WIDTH///
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!//
public class CustomTabLayout extends HorizontalScrollView {
    ...
    private static final int DEFAULT_HEIGHT = 58; // dps
    ...
    private static final Pools.Pool<CustomTabLayout.Tab> sTabPool = new Pools.SynchronizedPool<>(16);
    ...
    private final ArrayList<CustomTabLayout.Tab> mTabs = new ArrayList<>();
    private CustomTabLayout.Tab mSelectedTab;
    ...
    private CustomTabLayout.OnTabSelectedListener mSelectedListener;
    private final ArrayList<CustomTabLayout.OnTabSelectedListener> mSelectedListeners = new ArrayList<>();
    private CustomTabLayout.OnTabSelectedListener mCurrentVpSelectedListener;
    ...
    private CustomTabLayout.AdapterChangeListener mAdapterChangeListener;
    private boolean mSetupViewPagerImplicitly;

    // Pool we use as a simple RecyclerBin
    private final Pools.Pool<CustomTabLayout.TabView> mTabViewPool = new Pools.SimplePool<>(12);
...
//        final CustomTabLayout.Tab tab = newTab();
//        if (item.mText != null) {
...
//    }
    ...
    /**
     * @deprecated Use {@link #addOnTabSelectedListener(CustomTabLayout.OnTabSelectedListener)} and
     * {@link #removeOnTabSelectedListener(CustomTabLayout.OnTabSelectedListener)}.
     */
    @Deprecated
    public void setOnTabSelectedListener(@Nullable CustomTabLayout.OnTabSelectedListener listener) {
     ...
     * {@link #removeOnTabSelectedListener(CustomTabLayout.OnTabSelectedListener)}.</p>
     *
     * @param listener listener to add
     */
    public void addOnTabSelectedListener(@NonNull CustomTabLayout.OnTabSelectedListener listener) {
     ...
     * {@link #addOnTabSelectedListener(CustomTabLayout.OnTabSelectedListener)}.
     *
     * @param listener listener to remove
     */
    public void removeOnTabSelectedListener(@NonNull CustomTabLayout.OnTabSelectedListener listener) {
    ...
    /**
     * Create and return a new {@link CustomTabLayout.Tab}. You need to manually add this using
     * {@link #addTab(CustomTabLayout.Tab)} or a related method.
     *
     * @return A new Tab
     * @see #addTab(CustomTabLayout.Tab)
     */
    @NonNull
    public CustomTabLayout.Tab newTab() {
        CustomTabLayout.Tab tab = sTabPool.acquire();
        if (tab == null) {
            tab = new Tab();
        }
        tab.mParent = this;
    ...
    public void removeTabAt(int position) {
        ...
        final CustomTabLayout.Tab removedTab = mTabs.remove(position);
        if (removedTab != null) {
            removedTab.reset();
    ...
    public void removeAllTabs() {
        ...
        for (final Iterator<CustomTabLayout.Tab> i = mTabs.iterator(); i.hasNext(); ) {
            final CustomTabLayout.Tab tab = i.next();
            i.remove();
            tab.reset();
     ...
     */
    public void setTabMode(@CustomTabLayout.Mode int mode) {
        if (mode != mMode) {
            mMode = mode;
            applyModeAndGravity();
        }
     ...
     */
    public void setTabGravity(@CustomTabLayout.TabGravity int gravity) {
     ...
     * The one-stop shop for setting up this {@link CustomTabLayout} with a {@link ViewPager}.
     * <p>
     * <p>This method will link the given ViewPager and this CustomTabLayout together so that
    ...
    private void setupWithViewPager(@Nullable final ViewPager viewPager, boolean autoRefresh,
        ...
        if (viewPager != null) {
            ...
            if (mPageChangeListener == null) {
                mPageChangeListener = new CustomTabLayoutOnPageChangeListener(this);
            }
    ...
    /**
     * @deprecated Use {@link #setupWithViewPager(ViewPager)} to link a CustomTabLayout with a ViewPager
     * together. When that method is used, the CustomTabLayout will be automatically updated
    ...
    private CustomTabLayout.TabView createTabView(@NonNull final CustomTabLayout.Tab tab) {
        CustomTabLayout.TabView tabView = mTabViewPool != null ? mTabViewPool.acquire() : null;
        if (tabView == null) {
    ...
    private void addTabView(CustomTabLayout.Tab tab) {
        final CustomTabLayout.TabView tabView = tab.mView;
...
//    }
    ...
    private void removeTabViewAt(int position) {
        final CustomTabLayout.TabView view = (CustomTabLayout.TabView) mTabStrip.getChildAt(position);
    ...
    private void ensureScrollAnimator() {
        if (mScrollAnimator == null) {
            ...
            mScrollAnimator.setInterpolator(AnimationUtils.INSTANCE.getFAST_OUT_SLOW_IN_INTERPOLATOR());
    ...
    void selectTab(final CustomTabLayout.Tab tab, boolean updateIndicator) {
        final CustomTabLayout.Tab currentTab = mSelectedTab;
        ...
        if (currentTab == tab) {
            ...
            final int newPosition = tab != null ? tab.getPosition() : CustomTabLayout.Tab.INVALID_POSITION;
            if (updateIndicator) {
                if ((currentTab == null || currentTab.getPosition() == CustomTabLayout.Tab.INVALID_POSITION)
                        && newPosition != CustomTabLayout.Tab.INVALID_POSITION) {
    ...
    private void dispatchTabSelected(@NonNull final CustomTabLayout.Tab tab) {
        for (int i = mSelectedListeners.size() - 1; i >= 0; i--) {
            mSelectedListeners.get(i).onTabSelected(tab);
        }
    }

    private void dispatchTabUnselected(@NonNull final CustomTabLayout.Tab tab) {
        for (int i = mSelectedListeners.size() - 1; i >= 0; i--) {
            mSelectedListeners.get(i).onTabUnselected(tab);
        }
    }

    private void dispatchTabReselected(@NonNull final CustomTabLayout.Tab tab) {
        for (int i = mSelectedListeners.size() - 1; i >= 0; i--) {
    ...
    class TabView extends LinearLayout {
        ...
        void setTab(@Nullable final CustomTabLayout.Tab tab) {
            if (tab != mTab) {
                mTab = tab;
                update();
            }
        ...
        final void update() {
            final CustomTabLayout.Tab tab = mTab;
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            if (startLeft != targetLeft || startRight != targetRight) {
                ...
                animator.setInterpolator(AnimationUtils.INSTANCE.getFAST_OUT_SLOW_IN_INTERPOLATOR());
                animator.setDuration(duration);
                animator.setFloatValues(0, 1);
                animator.addUpdateListener(animator1 -> {
                    final float fraction = animator1.getAnimatedFraction();
                    setIndicatorPosition(
                            AnimationUtils.INSTANCE.lerp(startLeft, targetLeft, fraction),
                            AnimationUtils.INSTANCE.lerp(startRight, targetRight, fraction));
                });
    ...
    private int getDefaultHeight() {
        ...
        for (int i = 0, count = mTabs.size(); i < count; i++) {
            CustomTabLayout.Tab tab = mTabs.get(i);
     ...
     * kept in sync.
     * <p>
     * <p>This class stores the provided CustomTabLayout weakly, meaning that you can use
    ...
    public static class CustomTabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<CustomTabLayout> mCustomTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public CustomTabLayoutOnPageChangeListener(CustomTabLayout CustomTabLayout) {
            mCustomTabLayoutRef = new WeakReference<>(CustomTabLayout);
        }
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            final CustomTabLayout CustomTabLayout = mCustomTabLayoutRef.get();
